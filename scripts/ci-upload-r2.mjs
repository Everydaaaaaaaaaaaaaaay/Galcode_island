...
// 同步上传到 Cloudflare R2。安装包文件名带版本号 = 永久缓存；latest.json 短缓存覆盖。
//
// 输入：
//   artifacts/<job>/*.dmg / *.exe / *.tar.gz / *.sig   各平台产物
//   dist-release/latest.r2.json                         合并后的 R2 索引
//
// 环境变量：
...
if (existsSync(artifactsDir)) {
  for (const jobDir of readdirSync(artifactsDir)) {
    ...
    for (const entry of readdirSync(base)) {
      ...
      if (entry === "latest.json") continue; // 各 job 的 fragment 不上传
...
// 去重（不同 job 可能都产同名文件；正常情况下唯一，这里兜底）
const uniqueUploads = [];
const seen = new Set();
