...
//   4. 生成 latest.json（仅含本平台 fragment；publish job 会合并所有平台）
...
// 临时目录，放入 .app + Applications 快捷方式（拖拽安装的标准布局）
...
runWithRetry("hdiutil", [
  ...
  "-ov",
  // UDZO 默认 zlib level 很低，对脚本/资源压缩率差；level 9 通常能省 30-40%，
  // 代价只是打包慢几秒。
  "-format", "UDZO",
...
// Tauri updater 协议约定的命名：<ProductName>.app.tar.gz
...
// ---------------------------------------------------------------------------
// 4. 生成 latest.json (单平台 fragment)
// ---------------------------------------------------------------------------

console.log("\n=== Step 4: Generate latest.json fragment ===");
...
// 从 updater endpoint 推断 GitHub Release 下载基础 URL（fallback）
